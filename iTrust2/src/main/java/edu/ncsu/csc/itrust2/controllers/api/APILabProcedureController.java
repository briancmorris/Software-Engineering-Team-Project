...
public class APILabProcedureController extends APIController {
    ...
    /**
     * /** Sets PUT mapping request to add a new lab procedure to an existing
     * office visit.
     *
     * @param officeVisitID
     ...
     *             officeVisitID,
     * @RequestBody final LabProcedureForm labForm ) { try { final LabProcedure
     *              lab = new LabProcedure( labForm ); final OfficeVisit ov =
     *              OfficeVisit.getById( officeVisitID ); if ( ov == null ) {
     *              LoggerUtil.log( TransactionType.LAB_PROCEDURE_ADD,
     *              LoggerUtil.currentUser(), "No office visit found with id " +
     ...
     *              officeVisitID ); return new ResponseEntity( ov,
     *              HttpStatus.OK ); } catch ( final Exception e ) {
     *              LoggerUtil.log( TransactionType.LAB_PROCEDURE_ADD,
     ...
     *                Long officeVisitID, @RequestBody final Long labID ) { try
     *                { final LabProcedure lab = LabProcedure.getById( labID );
     *                final OfficeVisit ov = OfficeVisit.getById( officeVisitID
     *                ); if ( ov == null ) { LoggerUtil.log(
     *                TransactionType.LAB_PROCEDURE_REMOVE,
     *                LoggerUtil.currentUser(), "No office visit found with id "
     *                + officeVisitID ); return new ResponseEntity(
     *                errorResponse( "No Office Visit found with id " +
     *                officeVisitID ), HttpStatus.NOT_FOUND ); } if ( lab ==
     *                null ) { LoggerUtil.log(
     *                TransactionType.LAB_PROCEDURE_REMOVE,
     ...
     *                + labID + " from office visit found with id " +
     *                officeVisitID ); return new ResponseEntity( ov,
     *                HttpStatus.OK ); } catch ( final Exception e ) {
     *                LoggerUtil.log( TransactionType.LAB_PROCEDURE_REMOVE,
     *                LoggerUtil.currentUser(), "Failed to remove lab procedure"
     *                ); return new ResponseEntity( errorResponse( "Failed to
     *                remove lab: " + e.getMessage() ), HttpStatus.BAD_REQUEST
     *                ); } }
     */
    /**
